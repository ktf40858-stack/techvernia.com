...
def add_missing_feature_translations(tool_key, tool_name):
    ...
    for lang, translations in feature_translations.items():
        # Find the language section pattern: "de": {
        ...
        # Check which translations are missing and add them
        lines_to_add = []
        for key_suffix, translation in translations.items():
            full_key = f"review.{tool_key}.{key_suffix}"

            # Check if this key already exists anywhere in the file
            if f'"{full_key}"' not in content:
...
print("=" * 70)
print("COMPLETING FEATURE TRANSLATIONS FOR ALL LANGUAGES")
print("=" * 70)
...
for tool_key, tool_name in translation_tools.items():
    ...
    if add_missing_feature_translations(tool_key, tool_name):
        print(f"  [+] Added missing feature translations")
        fixed_count += 1
    else:
