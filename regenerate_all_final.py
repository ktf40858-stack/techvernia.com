#!/usr/bin/env python3
"""Regenerate ALL i18n files after agent completion"""
...
def merge_all_translations(tool_name):
    """Merge English base + all batch files + individual language batches"""
    print(f"\n{tool_name}:")
    ...
    # Load individual language batch files (batch-ko.json, batch-hi.json, etc.)
    for lang in ['es', 'fr', 'de', 'pt', 'zh', 'ja', 'ko', 'ar', 'hi']:
        lang_file = os.path.join(ANALYTICS_DIR, f"{tool_name}-batch-{lang}.json")
        lang_data = load_json_file(lang_file)
        
        if lang_data and lang in lang_data:
            result[lang] = lang_data[lang]
