#!/usr/bin/env python3
"""
Script d'injection des traductions Claude dans i18n.js
"""
import json
import re

print("🚀 Injection des traductions Claude dans i18n.js...")

# 1. Charger les traductions Claude
print("\n📖 Chargement des traductions Claude...")
with open('claude_translations_all_langs.json', 'r', encoding='utf-8') as f:
    claude_translations = json.load(f)
...
# 4. Pour chaque langue, ajouter les traductions Claude
languages_map = {
    'es': 'SPANISH',
    'fr': 'FRENCH',
    'de': 'GERMAN',
...
for lang_code, lang_name in languages_map.items():
    ...
    if lang_code not in claude_translations:
    ...
    # Générer les lignes de traduction Claude
    translation_lines = []
    translation_lines.append("\n        // ===== Claude Review Translations =====")

    for key, value in claude_translations[lang_code].items():
        escaped_value = escape_js_string(value)
        translation_lines.append(f'        "{key}": "{escaped_value}",')

    translation_lines.append("        // ===== End Claude Review Translations =====\n")

    translations_block = '\n'.join(translation_lines)

    # Insérer les traductions
    i18n_content = i18n_content[:insert_pos] + translations_block + i18n_content[insert_pos:]
    modifications_count += len(claude_translations[lang_code])

    print(f"  ✅ {len(claude_translations[lang_code])} clés injectées")
...
print("\n" + "="*60)
print("✅ INJECTION CLAUDE TERMINÉE!")
print(f"📊 {modifications_count} traductions Claude ajoutées dans i18n.js")
print("="*60)
