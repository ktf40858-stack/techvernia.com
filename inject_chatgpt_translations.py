#!/usr/bin/env python3
"""
Script d'injection des traductions ChatGPT dans i18n.js
"""
import json
import re

print("🚀 Injection des traductions ChatGPT dans i18n.js...")

# 1. Charger les traductions ChatGPT
print("\n📖 Chargement des traductions ChatGPT...")
with open('chatgpt_translations_all_langs.json', 'r', encoding='utf-8') as f:
    chatgpt_translations = json.load(f)
...
# 4. Pour chaque langue, ajouter les traductions ChatGPT
languages_map = {
    'es': 'SPANISH',
    'fr': 'FRENCH',
    'de': 'GERMAN',
...
for lang_code, lang_name in languages_map.items():
    ...
    # Générer les lignes de traduction ChatGPT
    translation_lines = []
    translation_lines.append("\n        // ===== ChatGPT Review Translations =====")

    for key, value in chatgpt_translations[lang_code].items():
        escaped_value = escape_js_string(value)
        translation_lines.append(f'        "{key}": "{escaped_value}",')

    translation_lines.append("        // ===== End ChatGPT Review Translations =====\n")

    translations_block = '\n'.join(translation_lines)

    # Insérer les traductions
    i18n_content = i18n_content[:insert_pos] + translations_block + i18n_content[insert_pos:]
    modifications_count += len(chatgpt_translations[lang_code])

    print(f"  ✅ {len(chatgpt_translations[lang_code])} clés injectées")
...
print(f"📊 {modifications_count} traductions ChatGPT ajoutées dans i18n.js")
print("="*60)
