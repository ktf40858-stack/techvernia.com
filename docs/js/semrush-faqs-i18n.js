...
function getSemrushFaqTranslation(key, lang) {
    if (semrushFaqTranslations[lang] && semrushFaqTranslations[lang][key]) {
        return semrushFaqTranslations[lang][key];
    }
    return null;
}
...
function applySemrushFaqTranslations(lang) {
    ...
    document.querySelectorAll('[data-i18n^="review.semrush.faq"]').forEach(element => {
        ...
        if (key) {
            const translation = getSemrushFaqTranslation(key, lang);
            if (translation) {
