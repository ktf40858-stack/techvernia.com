#!/usr/bin/env python3
"""
REAL TRANSLATOR - AUTO START
Automatically starts translating without confirmation
"""
...
def translate_text(text, target_lang, cache):
    ...
    if any(marker in text for marker in ['()', '{}', '[]', '<>', '//', '/*', 'GPT-', 'API', 'http']):
        cache[cache_key] = text
        return text
...
def main():
    ...
    print("="*70)
    print("  REAL TRANSLATOR - AUTO MODE")
    print("  Translating 18,425 keys in 9 languages")
    print("="*70)
    ...
    print(f"   Estimated time: {(total_keys * len(LANGUAGES) * 0.6) / 3600:.1f} hours")
    ...
    for i, (key, en_text) in enumerate(keys_list):
        ...
        if key in completed:
            ...
            for lang_code, api_lang in LANGUAGES.items():
                cache_key = f"{en_text[:100]}|{api_lang}"
                if cache_key in cache:
        ...
        if (i + 1) % 10 == 0:
            print(f"[{i+1}/{total_keys}] {(i+1)/total_keys*100:.1f}% | ETA: {eta_hours:.1f}h | {key[:50]}")
        ...
        if (i + 1) % 50 == 0:
            ...
            print(f"  💾 Progress saved ({len(completed)}/{total_keys}) - Cache: {len(cache)}")
    ...
    print(f"{'='*70}")
    print(f"\n📌 Next step: Run inject_real_translations.py to update i18n.js")
