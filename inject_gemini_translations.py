#!/usr/bin/env python3
"""
Script d'injection des traductions Gemini dans i18n.js
"""
import json
import re

print("🚀 Injection des traductions Gemini dans i18n.js...")

# 1. Charger les traductions Gemini
print("\n📖 Chargement des traductions Gemini...")
with open('gemini_translations_all_langs.json', 'r', encoding='utf-8') as f:
    gemini_translations = json.load(f)
...
# 4. Pour chaque langue, ajouter les traductions Gemini
languages_map = {
    'es': 'SPANISH',
    'fr': 'FRENCH',
    'de': 'GERMAN',
...
for lang_code, lang_name in languages_map.items():
    ...
    if lang_code not in gemini_translations:
    ...
    # Générer les lignes de traduction Gemini
    translation_lines = []
    translation_lines.append("\n        // ===== Gemini Review Translations =====")

    for key, value in gemini_translations[lang_code].items():
        escaped_value = escape_js_string(value)
        translation_lines.append(f'        "{key}": "{escaped_value}",')

    translation_lines.append("        // ===== End Gemini Review Translations =====\n")

    translations_block = '\n'.join(translation_lines)

    # Insérer les traductions
    i18n_content = i18n_content[:insert_pos] + translations_block + i18n_content[insert_pos:]
    modifications_count += len(gemini_translations[lang_code])

    print(f"  ✅ {len(gemini_translations[lang_code])} clés injectées")
...
print("\n" + "="*60)
print("✅ INJECTION GEMINI TERMINÉE!")
print(f"📊 {modifications_count} traductions Gemini ajoutées dans i18n.js")
print("="*60)
