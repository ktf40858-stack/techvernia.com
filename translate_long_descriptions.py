...
# Traductions professionnelles pour les textes longs clés
long_text_translations = {
    "{tool} is a leading AI-powered education platform designed to streamline workflows and enhance productivity. Built with cutting-edge artificial intelligence technology, it delivers exceptional performance for modern businesses.": {
        ...
        "ko": "{tool}는 워크플로우를 간소화하고 생산성을 향상시키도록 설계된 선도적인 AI 기반 교육 플랫폼입니다. 최첨단 인공지능 기술로 구축되어 현대 교육 기관에 탁월한 성능을 제공합니다.",
    ...
    "Whether you're a small startup or a large enterprise, {tool} scales to meet your needs while maintaining high performance and reliability.": {
        ...
        "pt": "Seja uma pequena startup ou uma grande empresa, {tool} se dimensiona para atender às suas necessidades, mantendo alto desempenho e confiabilidade.",
        "zh": "无论您是小型初创公司还是大型企业，{tool}都能扩展以满足您的需求，同时保持高性能和可靠性。",
        "ja": "小規模なスタートアップであろうと大企業であろうと、{tool}は高いパフォーマンスと信頼性を維持しながら、ニーズに合わせて拡張します。",
        "ko": "소규모 스타트업이든 대기업이든, {tool}는 높은 성능과 안정성을 유지하면서 귀하의 요구 사항을 충족하도록 확장됩니다.",
...
def get_tool_display(tool):
    return ' '.join(p.capitalize() for p in tool.split('-'))

def add_long_text_translations(tool):
    """Ajoute les traductions des textes longs"""
    ...
    tool_display = get_tool_display(tool)

    # Pour chaque texte long
    for en_template, translations in long_text_translations.items():
        en_text = en_template.replace("{tool}", tool_display)

        # Pour chaque langue
        for lang, trans_template in translations.items():
            trans_text = trans_template.replace("{tool}", tool_display)

            # Échapper les caractères spéciaux pour regex
            escaped_en = re.escape(en_text)

            # Remplacer dans le fichier JS
            # Pattern: "key": "English text"
            pattern = rf'("[^"]+"):\s*"{escaped_en}"'

            def replacer(match):
                key = match.group(1)
                return f'{key}: "{trans_text}"'

            js_content = re.sub(pattern, replacer, js_content)

    # Sauvegarder
    with open(js_file, 'w', encoding='utf-8') as f:
        f.write(js_content)

    print(f"  [OK] {tool}-i18n.js - Long texts translated")

# MAIN EXECUTION
print("=" * 70)
print("TRANSLATING LONG DESCRIPTION TEXTS")
print("=" * 70)

for tool in tools:
    add_long_text_translations(tool)

print("\n" + "=" * 70)
print("[COMPLETE] All long texts translated!")
print("=" * 70)
