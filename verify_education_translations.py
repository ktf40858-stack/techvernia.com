...
# MAIN
print("=" * 80)
print(" " * 20 + "EDUCATION I18N COMPLETION REPORT")
print("=" * 80)

print("\n" + "-" * 80)
print(f"{'Tool':<25} {'EN':<6} {'DE':<6} {'ES':<6} {'FR':<6} {'PT':<6} {'ZH':<6} {'JA':<6} {'KO':<6} {'AR':<6} {'HI':<6}")
print("-" * 80)
...
for tool in tools:
    ...
    if os.path.exists(js_file):
        ...
        # Afficher les stats
        print(f"{tool:<25} {stats['en']:<6} {stats['de']:<6} {stats['es']:<6} {stats['fr']:<6} {stats['pt']:<6} {stats['zh']:<6} {stats['ja']:<6} {stats['ko']:<6} {stats['ar']:<6} {stats['hi']:<6}")

        # Ajouter au total
        for lang, count in stats.items():
            total_stats[lang] += count
    else:
        print(f"{tool:<25} FILE NOT FOUND")

print("-" * 80)
print(f"{'TOTAL':<25} {total_stats['en']:<6} {total_stats['de']:<6} {total_stats['es']:<6} {total_stats['fr']:<6} {total_stats['pt']:<6} {total_stats['zh']:<6} {total_stats['ja']:<6} {total_stats['ko']:<6} {total_stats['ar']:<6} {total_stats['hi']:<6}")
print("=" * 80)
...
print(" " * 25 + "SUMMARY")
print("=" * 80)
print(f"Total Education Tools: {len(tools)}")
