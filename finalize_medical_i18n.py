#!/usr/bin/env python3
"""Finalize Medical i18n by merging batches and generating JS files."""
import json
import os
...
def merge_translations(tool_name):
    ...
    # Load FAQ batches
    faq_batch1 = load_json_safely(BASE_DIR / f"{tool_name}-faqs-batch1.json")
    faq_batch2 = load_json_safely(BASE_DIR / f"{tool_name}-faqs-batch2.json")
    faq_batch3 = load_json_safely(BASE_DIR / f"{tool_name}-faqs-batch3.json")
    ...
    for lang in ["es", "fr", "de"]:
        if lang in batch1:
            ...
            # Add FAQs
            if lang in faq_batch1:
                merged[lang].update(faq_batch1[lang])
            # Add common titles
    ...
    for lang in ["pt", "zh", "ja"]:
        if lang in batch2:
            ...
            # Add FAQs
            if lang in faq_batch2:
                merged[lang].update(faq_batch2[lang])
            # Add common titles
    ...
    for lang in ["ko", "ar", "hi"]:
        if lang in batch3:
            ...
            # Add FAQs
            if lang in faq_batch3:
                merged[lang].update(faq_batch3[lang])
            # Add common titles
...
def main():
    print("="*60)
    print("MEDICAL I18N FINALIZATION")
    print("="*60)
    ...
    print("\n" + "="*60)
    print(f"SUCCESS: {stats['success']}/{len(MEDICAL_TOOLS)}")
    if stats["failed"]:
