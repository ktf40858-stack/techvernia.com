...
# Questions FAQ communes (en anglais)
...
def add_faq_i18n_to_html(html_file, tool):
    ...
    # Pour chaque question FAQ dans les h4
...
def create_faq_translations(tool):
    """Crée les traductions FAQ pour un outil"""
    tool_display = tool_to_title(tool)
    tool_key = tool.replace("-", ".")
    ...
    for key_template, translations in faq_translations.items():
        ...
        for lang in languages:
            if lang in translations:
                ...
                all_translations[lang][full_key] = translation

    return all_translations

def append_to_i18n_file(tool, faq_translations):
    """Ajoute les traductions FAQ au fichier i18n.js existant"""
    ...
    for lang in languages:
        if lang not in faq_translations or not faq_translations[lang]:
            continue
        ...
        if match:
            ...
            # Ajouter les nouvelles traductions FAQ
            for key, value in faq_translations[lang].items():
                # Vérifier si la clé existe déjà
                if f'"{key}"' not in lang_section:
                    # Trouver le dernier élément pour ajouter après
                    # Chercher la dernière paire key:value avant la fermeture }
                    last_item = re.findall(r'"[^"]+"\s*:\s*"[^"]*"', lang_section)
                    if last_item:
                        last = last_item[-1]
                        # Échapper les caractères spéciaux dans value
                        escaped_value = value.replace('"', '\\"')
                        new_item = f',\n    "{key}": "{escaped_value}"'
                        lang_section = lang_section.replace(last, last + new_item)

            # Remplacer la section de langue dans le contenu
            content = content.replace(match.group(1), lang_section)
...
# MAIN
print("=" * 70)
print("ADDING FAQ QUESTIONS I18N TO SALES TOOLS")
print("=" * 70)
...
for tool in tools:
    ...
    if os.path.exists(html_file):
        # 1. Ajouter data-i18n aux questions FAQ dans HTML
        ...
        # 2. Créer les traductions FAQ
        faq_trans = create_faq_translations(tool)

        # 3. Ajouter au fichier i18n.js
        if append_to_i18n_file(tool, faq_trans):
            print(f"  [+] i18n.js: Added {len(faq_trans['en'])} FAQ question translations")
            total_js += 1
        else:
